...
#define HOST   "<YOUR_SERVER>"  // server's demain/IP
...
void loop() {
  ...
  if(!udp.socketReady(socket)) {
    ...
    udp.openSocket(UDP, HOST, PORT, socket);
  }
  ...
  if(curr_time-prev_time > interval) {
    ...
    udp.beginSend(socket);
    udp.send(message);
    udp.endSend();
    if(udp.waitRecv() == socket) {
      String response = udp.readRecv(socket); // get first 100 chars (max)
